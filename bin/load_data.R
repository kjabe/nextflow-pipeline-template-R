args <- commandArgs(trailingOnly = TRUE)
id_path <- args[1]
nulisa1_path <- args[2]
nulisa2_path <- args[3]

library(readxl)
library(dplyr)
library(tidyr)

# Read the data
id_data <- read_excel(id_path)
names(id_data)[2] <- "Group"

nulisa1_data <- read_excel(nulisa1_path)
nulisa2_data <- read_excel(nulisa2_path)

long_nulisa1 <- nulisa1_data %>%
  pivot_longer(cols = -targetName, names_to = "ID", values_to = "Value") %>%
  mutate(Batch = "Plate1")

long_nulisa2 <- nulisa2_data %>%
  pivot_longer(cols = -targetName, names_to = "ID", values_to = "Value") %>%
  mutate(Batch = "Plate2")

long_nulisa <- bind_rows(long_nulisa1, long_nulisa2)

all_dataset <- long_nulisa %>%
  left_join(id_data, by = "ID") %>%
  select(ID, Group, targetName, Value, Batch) %>%
  arrange(ID, targetName)

all_dataset <- all_dataset %>%
  mutate(Group = ifelse(is.na(Group), "NULISA Assay Control", Group))

# Save the dataset directly to the current working directory
saveRDS(all_dataset, "all_dataset.rds")